...
internal sealed class MudLinkButton : MudComponentBase
{
	private string Classname =>
		new CssBuilder("mud-typography mud-link")
			.AddClass($"mud-{Color.ToString().ToLowerInvariant()}-text")
			.AddClass($"mud-link-underline-{Underline.ToString().ToLowerInvariant()}")
			.AddClass($"mud-typography-{Typo.ToString().ToLowerInvariant()}")
